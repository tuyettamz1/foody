...
<div class="row">
	<div class="col-md-8 col-sm-12 col-xs-12">
		<h3 class="text-success"><i class="fa fa-edit"></i> Sửa địa điểm</h3>
		@if ($errors->any())
				...
				<div class="col-md-12 col-sm-12 col-xs-12">
					<form method="post" action="{{route('places.create')}}" enctype="multipart/form-data">
						...
						<div class="form-group ">
							...
							<input class="form-control" id="name" name="name" onkeyup="ChangeToSlug();" value="{{$place->name}}" />
						</div>						
						...
						<div class="form-group ">
							...
							<input class="form-control" name="address" type="text" value="{{$place->address}}"/>
						</div>															
						<input class="form-control" id="price_from" name="price_from" type="hidden" value="{{$place->price_from}}"/>			
						<input class="form-control" id="price_to" name="price_to" type="hidden" value="{{$place->price_to}}"/>					
						<div class="form-group ">
						...
						<div class="form-group ">
							...
							<select name="category" class="form-control">
								...
								<option value="{{$category->id}}" <?php if ($category->id==$place->category_id) echo "selected"; ?> >{{$category->name}}</option>
								@endforeach
							</select>
						</div>
						<div class="form-group ">
							...
							<select name="district" class="form-control">
								...
								<option value="{{$district->id}}" <?php if ($district->id==$place->district_id) echo "selected"; ?> >{{$district->name}}</option>
								@endforeach
							</select>
						</div>		
						<div class="form-group ">
							...
							<input class="form-control" name="phone" value="{{$place->phone}}"/>
						</div>
						<div class="form-group ">
							<label class="control-label">
								Chi tiết
							</label>
							<textarea class="form-control" name="detail">{{$place->detail}}</textarea>						
						</div>
						<div class="form-group ">
